...
export function ExperienceCanvas({ scene, capabilityFocus = 'overview', onAIPhase, ibiolPhase = 'today' }: { scene: number; capabilityFocus?: CapabilityFocus; onAIPhase?: (phase: 'certifications' | 'gh300' | 'concepts') => void; ibiolPhase?: IBIOLPhase }) {
  ...
  return <Canvas className="experience-canvas" dpr={[1, 1.6]} camera={{ position: [0, .15, 8.5], fov: 42 }} gl={{ antialias: true, powerPreference: 'high-performance', alpha: true }}>
    <Suspense fallback={null}>
      ...
      <CameraRig scene={scene} capabilityFocus={capabilityFocus} ibiolPhase={ibiolPhase} />
      <SceneEffects />
    </Suspense>
  </Canvas>
}
