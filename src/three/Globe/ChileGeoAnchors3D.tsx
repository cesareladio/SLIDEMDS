...
export interface ChileGeoAnchorSpec {
  id: string
  label: string
  lat: number
  lon: number
...
interface ChileGeoAnchors3DProps {
  anchors: ChileGeoAnchorSpec[]
  dimOpacity?: number
}

export function ChileGeoAnchors3D({ anchors, dimOpacity = 0.55 }: ChileGeoAnchors3DProps) {
  return <group>
    {anchors.map(anchor => {
      ...
      return (
        <group key={anchor.id} position={position}>
          ...
          <Html transform={false} center={false} style={{ pointerEvents: 'none' }}>
            <div className={`chile-geo3d-callout${anchor.isTop3 ? ' top3' : ''}`} style={{ opacity }}>
              <div className="chile-geo3d-label">
                <span className="chile-geo3d-name">{anchor.label}</span>
                {anchor.value !== undefined && (
                  <span className="chile-geo3d-value">{anchor.value.toLocaleString('es-CL')}</span>
                )}
                {anchor.tag && <span className="chile-geo3d-tag">{anchor.tag}</span>}
              </div>
            </div>
          </Html>
        </group>
